    ...
    def predictCharacter(self, character, num=0):
        ...
        if num:
            ...
            if predictedNumber==8 or predictedNumber==0:
                ...
                NN_prediction = self.numberConvModel.predict(np.expand_dims(tf.cast(cv2.resize(padded,None,fx=5,fy=5),tf.float32)))[0]
                predictedNumber=np.argmax(NN_prediction)*8
